...
let getHomepage = async (req, res) => {
  const [rows, fields] = await pool.pool.execute("SELECT * FROM `smartcard`");
  return res.render("index.ejs", { datacardid: rows });
};

let cardId = async (req, res) => {
  let cardid = req.params.cardid;
  const [user] = await pool.pool.execute(
...
let handleUploadFile = async (req, res) => {
  ...
  if (req.file) {
    ...
    await pool.pool.execute("update smartcard set avatar = ? where cardid = ?", [
      avatarsrc,
      cardid,
    ]);
  }
  if (password != "") {
    ...
    await pool.pool.execute("update smartcard set password = ? where cardid = ?", [
      hash,
      cardid,
    ]);
  }
...
let addcardidtodatabase = async (req, res) => {
  ...
  if (cardidgenerated!=""){
    await pool.pool.execute("insert into smartcard(cardid) values(?)",[cardidgenerated]);
    req.flash("messages", [
